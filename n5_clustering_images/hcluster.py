...
def simple_example():
    ...
    for c in clusters:
        print(c.get_cluster_elements())     # print out the id


def example_cluster_images():
    # create a list of images
    path = 'images/'
    imlist = imtools.get_imlist(path)

    # extract feature vector (8 bins per color channel)
    features = zeros([len(imlist), 512])    # 8x8x8
    for i, f in enumerate(imlist):
        im = array(Image.open(f))

        # multi-dimensional histogram
        # reshape the image so that one color channel in a row
    ...
    # visualize clusters with a threshold set to 25% of the maximum node distance in the tree
